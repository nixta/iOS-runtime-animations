...
class DrawAnimationViewController: AnimationDemoViewController {
    ...
    private var lineTraceAnimationHelper:AnimateLineTraceHelper!
    ...
    @IBAction func animate() {
        if let _ = self.generatedRoute?.routeGeometry {
            ...
            self.lineTraceAnimationHelper = AnimateLineTraceHelper(polyline: self.routeGraphic.geometry as! AGSPolyline, animatingGraphic: self.routeGraphic, speed: 2000)
            
            self.routeGraphic.symbol = self.routeSymbol(false)
    
            self.lineTraceAnimationHelper.startAnimation()
        }
